//retrieval_version13：设计两种cue，每种25次，频率分别为5kHz 和 10kHz。每次cue之间的时间间隔随机在10-30秒。shock在cue中间or结束前
...
// 定义随机序列参数
const int totalCount = 10;     // 总次数（必须是偶数）
...
void loop(){
  ...
  if(digitalRead(inputpin1) == HIGH){
    ...
    if(sequence[i] == 1){
      Serial.println("tone1");  //便于查看输出结果
      Serial.println(trial1);
      Serial.println("间隔时间:");
      Serial.println(randtime);
      digitalWrite(analogpin1,HIGH);   //输出一个HIGH信号
      digitalWrite(buttonpin3, LOW);  //不给与电刺激，保证处于一个低电平状态
      Up_Frequency();     //降频函数：执行时间2秒
      Up_Frequency();
      Up_Frequency();
      Up_Frequency();
      Up_Frequency();
      ...
      Up_Frequency();
      Up_Frequency();
      digitalWrite(analogpin1, LOW);   //恢复cue信号低电平状态
    ...
    if(sequence[i] == 0){
      ...
      Serial.println("间隔时间:");
      Serial.println(randtime);
      digitalWrite(analogpin2,HIGH);   //输出一个高电压信号
      digitalWrite(buttonpin3, LOW);  //不给与电刺激，保证处于一个低电平状态
      ...
      Down_Frequency();
      Down_Frequency();
      digitalWrite(analogpin2,LOW);  //恢复低电压信号
      delay(randtime*1000);   //两个trial之间的间隔时间
      trial2 = trial2 + 1;
      }
    }
    Serial.println("All trials end!");
    Serial.println(i);
    delay(180000);    //一系列trial结束后间隔时间，可以选择停止，也可以选择合适的间隔时间进行下一次trial
